...
CY_ISR(isr_UART_Interrupt)
{
    ...
    /* PSoC3 ES1, ES2 RTC ISR PATCH  */ 
        ...
        #if((CYDEV_CHIP_REVISION_USED <= CYDEV_CHIP_REVISION_3A_ES2) && (isr_UART__ES2_PATCH ))      
            isr_UART_ISR_PATCH();
...
{
    CY_SET_REG16(isr_UART_INTC_VECTOR, (uint16) address);
}
...
{
    return (cyisraddress) CY_GET_REG16(isr_UART_INTC_VECTOR);
}
...
uint8 isr_UART_GetState(void) 
{
    ...
    return ((*isr_UART_INTC_SET_EN & (uint8)isr_UART__INTC_MASK) != 0u) ? 1u:0u;
}
